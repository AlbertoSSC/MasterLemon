console.log("******************************** 301 - Calificacions Summary");
...
;
var studentMean = function (calification) {
    var sum = calification.reduce(function (acc, calification) { return acc + calification; }, 0);
    var mean = sum / calification.length;
    return mean.toPrecision(3);
};
var summarizeClassRoom = function (studentList) {
    return studentList.map(function (student) { return ({
        name: student.name,
        highestCalification: Math.max.apply(Math, student.califications),
        mean: studentMean(student.califications),
    }); });
};
